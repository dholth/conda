...
from conda.plan import (
    ...
    _update_old_plan as update_old_plan,
)
...
@pytest.mark.xfail(
    strict=True,
    reason="Not reporting link type until refactoring display_actions " "after txn.verify()",
)
...
def generate_mocked_resolve(pkgs, install=None):
    ...
    mock_resolve = namedtuple(
        "Resolve",
        ["get_dists_for_spec", "index", "explicit", "install", "package_name", "dependency_sort"],
    )
...
def generate_mocked_context(prefix, root_prefix, envs_dirs):
    ...
    return mocked_context(
        prefix=prefix, root_prefix=root_prefix, envs_dirs=envs_dirs, prefix_specified=False
    )
