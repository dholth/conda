...
def test_channels_add_empty_with_defaults():
    ...
    with make_temp_condarc() as rc:
        ...
        assert stdout == ""
        assert (
            stderr.strip() == "Warning: 'defaults' already in 'channels' list, moving to the top"
        )
...
def test_channels_add_duplicate():
    ...
    with make_temp_condarc(channels_initial) as rc:
        ...
        assert stdout == ""
        assert (
            stderr.strip() == "Warning: 'mychannel' already in 'channels' list, moving to the top"
        )
...
def test_get_channels_list():
    with make_temp_condarc(CONDARC_BASE) as rc:
        stdout, stderr, _ = run_command(Commands.CONFIG, "--file", rc, "--get", "channels")
        assert stdout == dedent(
            """\
            --add channels 'defaults'   # lowest priority
            --add channels 'test'   # highest priority
...
def test_get_boolean_value():
    with make_temp_condarc(CONDARC_BASE) as rc:
        stdout, stderr, _ = run_command(Commands.CONFIG, "--file", rc, "--get", "changeps1")
...
def test_get_string_value():
    with make_temp_condarc(CONDARC_BASE) as rc:
        stdout, stderr, _ = run_command(Commands.CONFIG, "--file", rc, "--get", "channel_alias")
...
def test_get_map_full():
    with make_temp_condarc(CONDARC_MAPS) as rc:
        stdout, stderr, _ = run_command(Commands.CONFIG, "--file", rc, "--get", "proxy_servers")
...
def test_get_unconfigured_key():
    with make_temp_condarc(CONDARC_BASE) as rc:
        stdout, stderr, _ = run_command(Commands.CONFIG, "--file", rc, "--get", "allow_softlinks")
        assert stdout == ""
        assert stderr == ""
...
def test_get_invalid_key():
    ...
    with make_temp_condarc(condarc) as rc:
        stdout, stderr, _ = run_command(
            Commands.CONFIG, "--file", rc, "--get", "invalid_key", use_exception_handler=True
        )
        assert stdout == ""
        assert stderr.strip() == "unknown key invalid_key"
...
def test_set_key():
    ...
    with make_temp_condarc(CONDARC_BASE) as rc:
        stdout, stderr, _ = run_command(Commands.CONFIG, "--file", rc, "--set", key, to_val)
        assert stdout == stderr == ""
...
def test_set_unconfigured_key():
    ...
    with make_temp_condarc(CONDARC_BASE) as rc:
        stdout, stderr, _ = run_command(
            Commands.CONFIG, "--file", rc, "--set", key, to_val, use_exception_handler=True
        )
        assert stdout == stderr == ""
...
def test_add_invalid_key():
    ...
    with make_temp_condarc(CONDARC_BASE) as rc:
        stdout, stderr, _ = run_command(
            Commands.CONFIG, "--file", rc, "--add", key, to_val, use_exception_handler=True
        )
        assert stdout == ""
        assert stderr.strip() == error
        assert _read_test_condarc(rc) == CONDARC_BASE
...
def test_remove_key():
    ...
    with make_temp_condarc(CONDARC_BASE) as rc:
        stdout, stderr, _ = run_command(
            Commands.CONFIG, "--file", rc, "--remove-key", key, use_exception_handler=True
        )
        assert stdout == stderr == ""
...
def test_set_check_types(key, str_value, py_value):
    with make_temp_condarc() as rc:
        stdout, stderr, _ = run_command(
            Commands.CONFIG, "--file", rc, "--set", key, str_value, use_exception_handler=True
        )
        assert stdout == stderr == ""
        with open(rc) as fh:
...
def test_set_and_get_bool():
    key = "restore_free_channel"
    with make_temp_condarc() as rc:
        stdout, stderr, _ = run_command(Commands.CONFIG, "--file", rc, "--set", key, "yes")
        stdout, stderr, _ = run_command(Commands.CONFIG, "--file", rc, "--get", key)
...
def test_ssl_verify_set_bool():
    with make_temp_condarc() as rc:
        stdout, stderr, _ = run_command(Commands.CONFIG, "--file", rc, "--set", "ssl_verify", "no")
        assert stdout == stderr == ""
        reset_context([rc])
...
def test_set_rc_without_user_rc():
    ...
    stdout, stderr, return_code = run_command(Commands.CONFIG, "--add", "channels", "test")
    assert stdout == stderr == ""
...
def test_custom_multichannels_prepend():
    with make_temp_condarc() as rc:
        stdout, stderr, return_code = run_command(
            Commands.CONFIG, "--file", rc, "--prepend", "custom_multichannels.foo", "bar"
        )
        assert stdout == stderr == ""
        assert _read_test_condarc(rc) == yaml_round_trip_dump(
            {"custom_multichannels": {"foo": ["bar"]}}
