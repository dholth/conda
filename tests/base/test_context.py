...
class ContextCustomRcTests(TestCase):
    ...
    def test_restore_free_channel(self):
        ...
        with env_var(
            "CONDA_RESTORE_FREE_CHANNEL", "true", stack_callback=conda_tests_ctxt_mgmt_def_pol
        ):
    ...
    def test_clobber_enum(self):
        with env_var(
            "CONDA_PATH_CONFLICT", "prevent", stack_callback=conda_tests_ctxt_mgmt_def_pol
        ):
    ...
    def test_threads(self):
        ...
        with env_var("CONDA_DEFAULT_THREADS", "3", stack_callback=conda_tests_ctxt_mgmt_def_pol):
        ...
        with env_var("CONDA_VERIFY_THREADS", "3", stack_callback=conda_tests_ctxt_mgmt_def_pol):
            assert context.default_threads == default_value
            assert context.verify_threads == 3
            assert context.repodata_threads == default_value
            assert context.execute_threads == 1

        with env_var("CONDA_REPODATA_THREADS", "3", stack_callback=conda_tests_ctxt_mgmt_def_pol):
            assert context.default_threads == default_value
            assert context.verify_threads == 1
            assert context.repodata_threads == 3
            assert context.execute_threads == 1

        with env_var("CONDA_EXECUTE_THREADS", "3", stack_callback=conda_tests_ctxt_mgmt_def_pol):
            assert context.default_threads == default_value
            assert context.verify_threads == 1
            assert context.repodata_threads == default_value
            assert context.execute_threads == 3
...
class ContextDefaultRcTests(TestCase):
    def test_subdirs(self):
        ...
        with env_var(
            "CONDA_SUBDIRS", ",".join(subdirs), stack_callback=conda_tests_ctxt_mgmt_def_pol
        ):
