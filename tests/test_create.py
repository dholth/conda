...
dependencies:
  ...
  - flask=1.0.3"""
    ...
    def test_pinned_override_with_explicit_spec(self):
        with make_temp_env("python=3.9") as prefix:
            run_command(Commands.CONFIG, prefix, "--add", "pinned_packages", "python=3.9.16")
                ...
                with env_var(
                    ...
                    with make_temp_channel(["flask-2.1.3"]) as channel:
                        ...
                        with patch.object(CondaSession, "get", autospec=True) as mock_method:
                            ...
                            assert not package_is_installed(prefix, "flask")
                            run_command(
                                Commands.INSTALL, prefix, "-c", channel, "flask", "--offline"
                            )
                            assert package_is_installed(prefix, "flask")
    ...
    def test_install_mkdir(self):
        try:
            ...
            run_command(
                Commands.INSTALL, prefix, "python-dateutil", "python", "--mkdir", no_capture=True
            )
            assert package_is_installed(prefix, "python")
