...
class CustomConfigChannelTests(TestCase):
    ...
    def test_migrated_custom_channels(self):
        ...
        assert channel.canonical_name == "darwin"
        assert (
            channel.url() == "https://some.url.somewhere/stuff/darwin/noarch/a-mighty-fine.tar.bz2"
        )
        assert channel.urls() == [
            "https://some.url.somewhere/stuff/darwin/noarch",
        ]
    ...
    def test_local_channel(self):
        ...
        try:
            with env_var(
                "CONDA_CROOT", conda_bld_path, stack_callback=conda_tests_ctxt_mgmt_def_pol
            ):
                Channel._reset_state()
...
class ChannelAuthTokenPriorityTests(TestCase):
    ...
    def test_named_custom_channel(self):
        ...
        assert channel.location == "another.url:8080/with/path"
        assert (
            channel.url() == "http://another.url:8080/with/path/chuck/label/dev/%s" % self.platform
        )
        assert (
            channel.url(True)
    ...
    def test_named_custom_channel_w_subchan(self):
        ...
        assert channel.location == "another.url:8080/with/path"
        assert (
            channel.url() == "http://another.url:8080/with/path/chuck/subchan/%s" % self.platform
        )
        assert (
            channel.url(True)
    ...
    def test_channel_alias(self):
        ...
        assert (
            channel.url(True) == "ftp://nm:ps@new.url:8082/t/zyx-wvut/charlie/%s" % self.platform
        )
        assert channel.urls() == [
...
class UrlChannelTests(TestCase):
    ...
    def test_subdir_env_var(self):
        with env_var(
            "CONDA_SUBDIR", "osx-1012-x84_64", stack_callback=conda_tests_ctxt_mgmt_def_pol
        ):
            channel = Channel(
