...
class MatchSpecTests(TestCase):
    ...
    def test_canonical_string_forms(self):
        ...
        assert (
            m("https://repo.anaconda.com/pkgs/free/linux-32::numpy") == "pkgs/free/linux-32::numpy"
        )
        assert (
    ...
    def test_channel_matching(self):
        with env_unmodified(conda_tests_ctxt_mgmt_def_pol):
            ...
            assert (
                ChannelMatch("https://conda.anaconda.org/conda-forge").match("conda-forge") is True
            )
            assert (
                ChannelMatch("conda-forge").match("https://conda.anaconda.org/conda-forge") is True
            )

            assert (
                ChannelMatch("https://repo.anaconda.com/pkgs/main").match("conda-forge") is False
            )
