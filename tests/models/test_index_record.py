...
class PrefixRecordTests(TestCase):
    def test_prefix_record_no_channel(self):
        with env_unmodified(conda_tests_ctxt_mgmt_def_pol):
            ...
            assert (
                pr.url == "https://repo.anaconda.com/pkgs/main/win-32/austin-1.2.3-py34_2.tar.bz2"
            )
            assert pr.channel.canonical_name == "defaults"
            assert pr.subdir == "win-32"
            assert pr.fn == "austin-1.2.3-py34_2.tar.bz2"
            channel_str = str(
                Channel("https://repo.anaconda.com/pkgs/main/win-32/austin-1.2.3-py34_2.tar.bz2")
            )
    ...
    def test_index_record_timestamp(self):
        ...
        rec = PackageRecord(
            name="test-package", version="1.2.3", build="2", build_number=2, timestamp=ts_secs
        )
