    ...
    def __call__(cls, *args, **kwargs):
        if len(args) == 1 and not kwargs:
            ...
            if "channels" in kwargs:
                ...
                name = kwargs["name"]
                channels = tuple(
                    super(ChannelType, cls).__call__(**_kwargs) for _kwargs in kwargs["channels"]
                )
...
class Channel(metaclass=ChannelType):
    ...
    def make_simple_channel(channel_alias, channel_url, name=None):
        ...
        if name and scheme:
            return Channel(
                scheme=scheme, auth=auth, location=test_url, token=token, name=name.strip("/")
            )
        if scheme:
            ...
            return Channel(
                scheme=scheme, auth=auth, location=location, token=token, name=name.strip("/")
            )
        else:
            return Channel(
                scheme=ca.scheme,
