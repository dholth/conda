...
DistDetails = namedtuple(
    "DistDetails", ("name", "version", "build_string", "build_number", "dist_name", "fmt")
)
        ...
        if len(args) == 1 and not kwargs:
            ...
            if value in Dist._cache_:
                ...
                dist = Dist.from_string(
                    value.repodata_record.fn, channel_override=value.channel.canonical_name
                )
    ...
    def parse_dist_name(string):
        ...
        try:
            ...
            build_number_as_string = "".join(
                filter(
                    lambda x: x.isdigit(), (build_string.rsplit("_")[-1] if build_string else "0")
                )
            )
