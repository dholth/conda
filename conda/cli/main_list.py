...
def list_packages(prefix, regex=None, format="human", show_channel_urls=None):
    ...
    installed = sorted(
        PrefixData(prefix, pip_interop_enabled=True).iter_records(), key=lambda x: x.name
    )
...
def print_packages(
    prefix, regex=None, format="human", piplist=False, json=False, show_channel_urls=None
):
    if not isdir(prefix):
