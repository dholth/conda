...
def execute_config(args, parser):
    ...
    if args.show is not None:
        ...
        if context.json:
            stdout_write(
                json.dumps(d, sort_keys=True, indent=2, separators=(",", ": "), cls=EntityEncoder)
            )
        else:
        ...
        if isfile(rc_path):
            ...
            if data:
                raise CondaError(
                    ...
                    "Use `conda config --describe` to display default configuration." % rc_path
                )
    ...
    grouped_paramaters = groupby(
        lambda p: context.describe_parameter(p)["parameter_type"], context.list_parameters()
    )
    ...
    for key, item in args.remove:
        ...
        if item not in rc_config[key]:
            ...
            raise CondaKeyError(key, f"{item!r} is not in the {key!r} key of the config file")
