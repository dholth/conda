...
class TooManyArgumentsError(ArgumentError):
    def __init__(self, expected, received, offending_arguments, optional_message="", *args):
...
class CondaHTTPError(CondaError):
    def __init__(
        self, message, url, status_code, reason, elapsed_time, response=None, caused_by=None
    ):
        # if response includes a valid json body we prefer the reason/message defined there
        try:
            body = response.json()
...
def maybe_raise(error, context):
    if isinstance(error, CondaMultiError):
        ...
        if (
            ...
            clobber_errors and context.path_conflict == PathConflict.warn and not context.clobber
        ):
