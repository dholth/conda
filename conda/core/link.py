...
def make_unlink_actions(transaction_context, target_prefix, prefix_record):
    ...
    directory_remove_actions = tuple(
        UnlinkPathAction(transaction_context, prefix_record, target_prefix, d, LinkType.directory)
...
class UnlinkLinkTransaction:
    ...
    def nothing_to_do(self):
        ...
        ) and all(is_conda_environment(stp.target_prefix) for stp in self.prefix_setups.values())
    ...
    def prepare(self):
        ...
        with Spinner(
            "Preparing transaction", not context.verbosity and not context.quiet, context.json
        ):
            for stp in self.prefix_setups.values():
                grps = self._prepare(
                    self.transaction_context,
    ...
    def _prepare(
        ...
        for pkg_info, lt, spec in zip(
            ...
            link_ag = ActionGroup(
                "link",
                pkg_info,
                cls._make_link_actions(transaction_context, pkg_info, target_prefix, lt, spec),
                target_prefix,
            )
            ...
            make_menu_ag = ActionGroup(
                "make_menus",
                pkg_info,
                MakeMenuAction.create_actions(transaction_context, pkg_info, target_prefix, lt),
                target_prefix,
            )
        ...
        register_action_groups = [
            ActionGroup("register", None, register_actions + history_actions, target_prefix)
        ]
                ...
                for path in target_short_paths:
                    ...
                    if path not in unlink_paths and lexists(join(target_prefix, path)):
                        ...
                        colliding_prefix_rec = first(
                            (
                                prefix_rec
                                for prefix_rec in PrefixData(target_prefix).iter_records()
                            ),
    ...
    def _verify_transaction_level(prefix_setups):
        ...
        conda_setups = tuple(
            setup for setup in prefix_setups.values() if setup.target_prefix in conda_prefixes
        )

        conda_unlinked = any(
            prec.name == "conda" for setup in conda_setups for prec in setup.unlink_precs
        )
        ...
        if conda_final_prefix in prefix_setups:
            for conda_dependency in conda_linked_depends:
                ...
                if dep_name not in pkg_names_being_lnkd and (
                    dep_name not in pkg_names_already_lnkd or dep_name in pkg_names_being_unlnkd
                ):
                    yield RemoveError(
    ...
    def _verify(self, prefix_setups, prefix_action_groups):
        ...
        for exc in self.verify_executor.map(
            UnlinkLinkTransaction._verify_individual_level, prefix_action_groups.values()
        ):
            if exc:
    ...
    def _execute(self, all_action_groups):
        ...
        with signal_handler(conda_signal_handler), time_recorder("unlink_link_execute"):
            exceptions = []
            with Spinner(
                "Executing transaction", not context.verbosity and not context.quiet, context.json
            ):
                ...
                for (group, register_group, install_side) in (
                    ...
                    if install_side:
                        ...
                        if individual_actions:
                            ...
                            composite_ag.append(
                                ActionGroup("compile", None, [composite], composite.target_prefix)
                            )
                    ...
                    register_actions = tuple(
                        group for group in all_action_groups if group.type == register_group
                    )
    ...
    def _execute_post_link_actions(axngroup):
        ...
        if prec:
            try:
                run_script(
                    target_prefix, prec, "post-unlink" if is_unlink else "post-link", activate=True
                )
            except Exception as e:  # this won't be a multi error
                # reverse this package
                reverse_excs = ()
    ...
    def _make_link_actions(
        transaction_context, package_info, target_prefix, requested_link_type, requested_spec
    ):
    ...
    def _change_report_str(self, change_report):
        ...
        if change_report.specs_to_remove:
            builder.append(
                "  removed specs:%s"
                % dashlist(sorted(str(s) for s in change_report.specs_to_remove), indent=4)
            )
            builder.append("")
    ...
    def _calculate_change_report(
        ...
        removed_precs = {
            namekey: unlink_map[namekey] for namekey in (unlink_namekeys - link_namekeys)
        }
        new_precs = {namekey: link_map[namekey] for namekey in (link_namekeys - unlink_namekeys)}
