...
def make_initialize_plan(
    ...
    if "powershell" in shells:
        ...
        config_powershell_paths = set(find_powershell_paths("powershell", "pwsh", "pwsh-preview"))
...
def init_fish_user(target_path, conda_prefix, reverse):
    ...
    if reverse:
        ...
        rc_content = re.sub(
            r"^\s*" + CONDA_INITIALIZE_RE_BLOCK, "", rc_content, flags=re.DOTALL | re.MULTILINE
        )
    else:
        if not on_win:
            rc_content = re.sub(
...
def modify_easy_install_pth(target_path, conda_source_root):
    ...
    new_contents = (
        easy_install_new_line + os.linesep + os.linesep.join(old_contents_lines) + os.linesep
    )
