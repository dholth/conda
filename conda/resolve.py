...
def _get_sat_solver_cls(sat_solver_choice=SatSolverChoice.PYCOSAT):
    ...
    for sat_solver in _sat_solvers.values():
        try:
            ...
            log.debug(
                "Attempted SAT interface '%s' but unavailable due to: %s", sat_solver_choice, e
            )
        else:
...
class Resolve:
    def __init__(self, index, processed=False, channels=()):
        ...
        self._system_precs = {
            _
            for _ in index
            if (hasattr(_, "package_type") and _.package_type == PackageType.VIRTUAL_SYSTEM)
        }
    ...
    def verify_specs(self, specs):
        ...
        bad_deps.extend(
            (spec,) for spec in non_tf_specs if (not spec.optional and not self.find_matches(spec))
        )
        if bad_deps:
    ...
    def _classify_bad_deps(self, bad_deps, specs_to_add, history_specs, strict_channel_priority):
        ...
        for chain in bad_deps:
            ...
            if (
                ...
                if python_first_specs:
                    ...
                    if not (
                        set(self.find_matches(python_spec)) & set(self.find_matches(chain[-1]))
                    ):
                        classes["python"].add(
                            (
        ...
        if classes["python"]:
            ...
            classes["direct"] = [
                _ for _ in classes["direct"] if _[1].startswith("python ") or len(_[0]) > 1
            ]
        return classes
    ...
    def build_conflict_map(self, specs, specs_to_add=None, history_specs=None):
        ...
        dep_list = {}
        with tqdm(
            total=len(specs), desc="Building graph of deps", leave=False, disable=context.json
        ) as t:
            for spec in specs:
        ...
        with tqdm(
            total=len(specs), desc="Determining conflicts", leave=False, disable=context.json
        ) as t:
    ...
    def get_reduced_index(self, explicit_specs, sort_by_exactness=True, exit_on_conflict=False):
        ...
        if log.isEnabledFor(DEBUG):
            log.debug(
                "Retrieving packages for: %s", dashlist(sorted(str(s) for s in explicit_specs))
            )
                ...
                if not filter_out.setdefault(prec, False):
                    ...
                    for ms in self.ms_depends(prec):
                        if not ms.optional and not any(
                            rec for rec in self.find_matches(ms) if not filter_out.get(rec, False)
                        ):
                ...
                while dep_specs:
                    ...
                    for dep_pkg in (_ for _ in self.find_matches(ms) if _ not in reduced_index2):
                        ...
                        if not strict_channel_priority or (
                            self._get_strict_channel(dep_pkg.name) == dep_pkg.channel.name
                        ):
    ...
    def push_MatchSpec(self, C, spec):
        ...
        tf = frozenset(
            _tf for _tf in (f.strip() for f in spec.get_exact_value("track_features") or ()) if _tf
        )
    ...
    def generate_spec_constraints(self, C, specs):
        ...
        if log.isEnabledFor(DEBUG):
            log.debug(
                "generate_spec_constraints returning with clause count: %d", C.get_clause_count()
            )
        return result
    ...
    def get_conflicting_specs(self, specs, explicit_specs):
        ...
        if solution:
            ...
            final_unsat_specs = tuple(
                minimal_unsatisfiable_subset(specs, sat=mysat, explicit_specs=explicit_specs)
            )
        else:
    ...
    def bad_installed(self, installed, new_specs):
        ...
        if not solution or xtra:
            ...
            if len(snames) < len(sat_name_map):
                limit = snames
                xtra = [rec for sat_name, rec in sat_name_map.items() if rec["name"] not in snames]
    ...
    def install_specs(self, specs, installed, update_deps=True):
        ...
        for prec in installed:
            ...
            if update_deps:
                ...
                spec = MatchSpec(name=name, version=version, build=build, channel=schannel)
            specs.insert(0, spec)
    ...
    def solve(
        ...
        def clean(sol):
            return [
                q for q in (C.from_index(s) for s in sol) if q and q[0] != "!" and "@" not in q
            ]
