...
def massage_arguments(arguments, errors="assert"):
    ...
    if isinstance(arguments, str):
        if errors == "assert":
            ...
            log.warning(
                "Please ensure arguments is not a string; " "used `shlex_split_unicode()` on it"
            )
...
def wrap_subprocess_call(
    root_prefix, prefix, dev_mode, debug_wrapper_scripts, arguments, use_system_tmp_path=False
):
